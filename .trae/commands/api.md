---
description: 推送接口文档到 Eolink Apikit（参数为接口描述，如「客户列表接口」）
---

执行接口推送流程，参数为：$ARGUMENTS

若没有本地配置（`eolink/eolink.config.json`），先引导用户运行 `eolink/setup.ps1`。
